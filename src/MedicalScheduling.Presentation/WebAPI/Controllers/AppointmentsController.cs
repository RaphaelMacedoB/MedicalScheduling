...
using MedicalScheduling.Application.Features.Appointments.Queries.GetAppointments;
...
public sealed class AppointmentsController : ControllerBase
{
  ...
  public async Task<IActionResult> GetAppointments([FromQuery] AppointmentFilter filter, CancellationToken ct)
  {
    var result = await _mediator.Send(new GetAppointmentsQuery(filter), ct);
      ...
      CancellationToken ct)
  {
    var result = await _mediator.Send(
        new GetAppointmentsQuery(filter with { DoctorId = doctorId }),
        ct);
  ...
  {
    var result = await _mediator.Send(
        new GetAppointmentsQuery(filter with { PatientId = patientId }),
        ct);
